...
export function useClientChatSocket(activeChatId: string | null): void {
  ...
  useEffect(() => {
      ...
      queryClient.setQueryData(["client", "chats"], (old: ClientChatSummary[] | undefined) =>
        old?.map((chat) => {
          if (chat.id === p.chat_id) {
            return {
              ...chat,
              unread_count: p.is_admin ? chat.unread_count + 1 : chat.unread_count,
              last_message_preview: p.content || "Attachment",
    ...
    const handleMsgUpdated = (p: { chat_id: string; message_id: string; content: string }) => {
      updateMessageInCache(p.chat_id, p.message_id, (m) => ({ ...m, content: p.content, is_edited: true }));
    };

    const handleMsgDeleted = (p: { chat_id: string; message_id: string }) => {
      updateMessageInCache(p.chat_id, p.message_id, (m) => ({ ...m, content: null, is_deleted: true, attachments: [] }));
    };
    ...
    const invalidate = () => queryClient.invalidateQueries({ queryKey: ["client", "chats"] });
...
export default useClientChatSocket;
