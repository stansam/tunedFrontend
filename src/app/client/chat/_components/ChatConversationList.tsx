    ...
    <div className="flex-1 flex flex-col overflow-hidden p-4 gap-3.5">
      <div className="flex gap-2 items-center shrink-0">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search conversations..." className="pl-8 text-xs h-8 bg-white/50 border-slate-200" />
        </div>
        <button onClick={() => setModalOpen(true)} className="h-8 px-3 rounded-lg bg-slate-900 text-white text-xs font-bold flex items-center gap-1.5 hover:bg-slate-800 active:scale-95 transition-all shadow-sm"><Plus className="h-3.5 w-3.5" /> New</button>
      </div>
      <div className="flex gap-1.5 border-b border-slate-100 pb-1.5 shrink-0">
        {(["all", "active", "closed"] as const).map((tab) => (
          <button key={tab} onClick={() => setFilter(tab)} className={`px-2.5 py-1 text-[10px] font-bold capitalize transition-all border-b-2 mb-[-8px] ${filter === tab ? "border-slate-900 text-slate-900" : "border-transparent text-slate-400 hover:text-slate-600"}`}>{tab}</button>
        ))}
      </div>
