...
import type { OrderFiltersState, StatusTab, SortField, SortOrder } from "../_types";
import { useDebounce } from "@/lib/hooks/index.hook";

const DEFAULT_SORT: SortField = "created_at";
const DEFAULT_ORDER: SortOrder = "desc";

export function parseFiltersFromParams(params: URLSearchParams): OrderFiltersState {
  return {
    status: (params.get("status") as StatusTab) || "all",
    q: params.get("q") || "",
...
export function useOrderFilters() {
  ...
  const pushParams = useCallback(
    (updates: Partial<OrderFiltersState>) => {
  ...
  const setStatus = useCallback(
    (status: StatusTab) => pushParams({ status, page: 1 }),
    [pushParams],
  );
