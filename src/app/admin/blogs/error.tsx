...
export default function BlogsError({ error, reset }: BlogsErrorProps) {
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.error("[AdminBlogsError] Caught error:", error);
    }
  }, [error]);
    ...
    <div className="flex min-h-[400px] flex-col items-center justify-center p-6 text-center">
      <div className="rounded-2xl border border-red-100 bg-red-50/50 p-8 max-w-md shadow-xs backdrop-blur-md">
        ...
        <p className="mt-2 text-xs text-slate-500 font-medium leading-relaxed">
          {error.message || "An unexpected error occurred while loading the blogs manager."}
        </p>
        <button
          onClick={reset}
