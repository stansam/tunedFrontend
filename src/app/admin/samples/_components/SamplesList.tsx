...
import type { SamplesListProps } from "../_props/samples.props";
  ...
  if (loading) {
    return (
      <div className="text-center py-12 text-slate-500 font-medium text-xs">
        Loading samples list... {/* TODO: Add custom skeleton */}
      </div>
    );
  }
  ...
  if (samples.length === 0) {
    return (
      <div className="rounded-xl border border-white/50 bg-white/40 backdrop-blur-md shadow-xs p-12 text-center text-slate-500 text-sm flex flex-col items-center justify-center gap-3">
        ...
        <p className="font-semibold text-slate-700">No samples found</p>
      ...
      <div className="space-y-3">
        {samples.map((sample) => (
          <SampleCard
            key={sample.id}
            sample={sample}
            expanded={expandedCardId === sample.id}
            onToggleExpand={() => onToggleExpandCard(sample.id)}
            onEdit={() => onEdit(sample)}
            onDelete={() => onDelete(sample)}
            onFeatureToggle={() => onFeatureToggle(sample.id, !sample.featured)}
          />
        ))}
      </div>
