...
import { useCardHover } from "../../_hooks/useCardHover";
import type { HowItWorksCardProps } from "../../_props/howItworks.props";

type CSSVarString = `${number}px`;
type WrapperStyle = MotionStyle & {
  "--x": CSSVarString | ReturnType<typeof useCardHover>["xTemplate"];
  "--y": CSSVarString | ReturnType<typeof useCardHover>["yTemplate"];
};
const transitionPreset = { duration: 0.3, ease: [0.22, 1, 0.36, 1] as const };

export function HowItWorksCard({ children, step, stepsItems }: HowItWorksCardProps) {
  const { xTemplate, yTemplate, handleMouseMove } = useCardHover();
  ...
  return (
    <motion.div
      ...
      style={{ "--x": xTemplate, "--y": yTemplate } as WrapperStyle}
    >
      <div
        ...
        style={{ background: "radial-gradient(600px circle at var(--x) var(--y), rgba(251,191,36,0.07), transparent 70%)" }}
        aria-hidden="true"
      />
      ...
      <div className="relative w-full overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm transition-shadow duration-300 group-hover:shadow-md">
        <div className="grid min-h-[460px] grid-cols-1 md:grid-cols-2">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <motion.div
                  ...
                  transition={{ delay: 0.05, ...transitionPreset }}
                >
                ...
                <motion.h3
                  ...
                  initial={{ opacity: 0, x: -16 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.1, ...transitionPreset }}
                >
                  {currentStep?.title}
                </motion.h3>
                <motion.p
                  ...
                  initial={{ opacity: 0, x: -16 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.15, ...transitionPreset }}
                >
          ...
          <div className="relative h-64 md:h-auto">{children}</div>
        </div>
      </div>
    </motion.div>
  );
